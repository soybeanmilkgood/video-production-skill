...
├── slides_prompts.json   ← 3 image prompts + shared style block (Path A)
└── (config.json)         ← copy from ../../references/config-example.json and fill in
```

To actually produce it (from this directory, services running):

```bash
cp ../../references/config-example.json config.json   # then set tts.voice
...
node ../../scripts/tts_with_asr.js                    # → audio/slide_01..03.wav (ASR-gated)
