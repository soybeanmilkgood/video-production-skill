...
### #14 從 ERNIE-Image-Turbo 遷移到 Z-Image-Turbo

**動機：** ERNIE-Image-Turbo 需要 50 steps（~50s/張），30 張投影片 = 25 分鐘。Z-Image-Turbo 是蒸餾模型，只需 8 steps（~8s/張），速度提升 6 倍。

**模型組合變更：**
| 項目 | ERNIE（舊） | Z-Image（新） |
|------|------------|--------------|
| Diffusion | ernie-image-turbo-Q4_K_M.gguf | z_image_turbo-Q4_K.gguf |
| VAE | flux2-vae.safetensors | ae.safetensors |
| Text encoder | ministral-3-3b.safetensors | Qwen3-4B-UD-Q4_K_XL.gguf |
| Steps | 50 | 8 |
| cfg-scale | 3.0 | 1.0 |

**關鍵差異：**
- cfg-scale 必須為 1.0（Z-Image 的訓練特性，更改會嚴重降質）
- 8 steps 即可生成高品質圖片
- `--diffusion-fa` flash attention 可進一步加速
- VAE 從 flux2-vae 換成 ae.safetensors（不同模型配不同 VAE）

**教訓：** 蒸餾模型（Turbo 系列）的 steps 需求遠低於標準模型。
換模型時一定要查清楚：VAE 配對、cfg-scale、建議 steps。
每個擴散模型都有自己的「脾氣」。

### #15 TTS Voice Clone：從 CustomVoice 遷移到 Base 模型

**動機：** 預設聲音（vivian）缺乏個人特色。Voice Clone 可從參考音檔提取聲紋，自訂音色。

**變更：**
| 項目 | CustomVoice（舊） | Voice Clone（新） |
|------|-------------------|-------------------|
| 模型 | qwen3-tts-customvoice | qwen3-tts-base17b |
| API | generate_custom_voice(speaker=) | generate_voice_clone(prompt=) |
| 參考音檔 | 不需要 | 5–30s 目標人聲 |
| 啟動流程 | 直接載入 | 載入 + 預載 clone prompt |

**關鍵設計：**
- clone_prompt 在 server 啟動時建立一次，後續合成直接使用
- 環境變數 VOICE_CLONE_REF / VOICE_CLONE_REF_TEXT 支援熱切換
- API 介面不變（POST /v1/audio/speech），pipeline 腳本幾乎不用改

**注意事項：**
- 參考音檔必須清晰無背景音，否則聲紋提取品質差
- ref_text 非必要但建議提供（聲紋更準確）
- base17b 模型與 customvoice 模型不能互換 API
- 更換參考音檔後必須重啟 TTS server

**教訓：** clone_prompt 預載入是關鍵優化。若每次請求都重新建立，會多 ~3s 延遲。
預載後每次合成 ~3-10s（含模型 warmup），與 CustomVoice 速度相當。

**教訓：** ForcedAligner 的詞級時間戳是精準的，但直接逐詞生成 SRT 會有碎片。需要後處理：合併碎片 + 修正邊界 + 控制行寬。

### #16 vLLM-Omni TTS：自架 server → 原生 serving（66x 加速）

**問題：** 自架 tts_server.py 逐句呼叫 generate_voice_clone()，
10 句 ~15 min。瓶頸：無 GPU 批次、每句重算 speaker embedding、
無 CUDA Graph、Python overhead。

**解法：** 遷移到 vLLM-Omni 原生 serving，三層優化：

1. **vLLM-Omni engine**：兩階段架構（Talker + Code2Wav），
   動態批次調度 + CUDA Graph capture（81% 命中率）
2. **Precomputed voice**：離線計算 papaya speaker profile（ICL 模式），
   啟動時自動載入，不需每次傳 ref_audio
3. **Warmup 機制**：首句觸發 Triton 編譯 + CUDA Graph capture，
   之後每句 1.35s 穩定輸出

**實測數據：**
| 情境 | 自架 server | vLLM-Omni | 倍數 |
|------|:-----------:|:---------:|:----:|
| 首句（含編譯） | ~150s | 55.8s | 2.7x |
| 後續每句（warm） | ~10-20s | 1.35s | 10-15x |
| 10 句總計 | ~900s | ~13.5s | 66x |

**注意事項：**
- 首次請求慢（Triton 編譯 + CUDA Graph capture ~55s）
- pipeline 自動 warmup（送短句「暖機測試。」觸發）
- precompute 用 --mode icl（有 ref_text）品質最好
- Base 模型只能 voice cloning，不能用預設聲音
- GPU memory 需與 sd-server 共享（gpu_memory_utilization: 0.25）

**教訓：** 自架 Python server 無法利用 vLLM 的
CUDA Graph + 動態批次 + Triton kernel 優化。
vLLM-Omni 的兩階段架構是 TTS 推理加速的正解。
Single request + warmup 是最穩定的實用方案。
